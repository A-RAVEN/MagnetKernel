...
void MGPipeLine::MakePipeline(VkDescriptorSetLayout* DescritorSetLayouts, uint32_t DescriptorSetLayoutsNum, VkRenderPass renderPass)
{
	...
	vertShaderModule = OwningRenderer->createShaderModule(vertShaderCode);
	fragShaderModule = OwningRenderer->createShaderModule(fragShaderCode);
	...
	pipelineLayoutInfo.setLayoutCount = DescriptorSetLayoutsNum; // Optional
	pipelineLayoutInfo.pSetLayouts = DescritorSetLayouts; // Optional
	...
	MGCheckVKResultERR(vkCreateGraphicsPipelines(OwningRenderer->LogicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline), "Pipeline创建失败");
...
void MGPipeLine::ReleasePipeline()
{
	if (pipeline != VK_NULL_HANDLE)
	{
		...
		vkDestroyPipeline(OwningRenderer->LogicalDevice, pipeline, nullptr);
		pipeline = VK_NULL_HANDLE;
	}

}

void MGPipeLine::CmdBindPipeline(VkCommandBuffer cmdBuffer)
{
